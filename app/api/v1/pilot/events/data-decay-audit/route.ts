...
export async function POST(request: NextRequest) {
  try {
    const rateLimit = await checkRateLimit(getClientIp(request), 60, 60 * 1000, "api:v1:pilot:events:data-decay-audit")
    ...
    const validation = PilotDataDecayAuditCreateSchema.safeParse(body)

    if (!validation.success) {
      return createApiError("Invalid data decay audit payload", 400, validation.error.flatten())
    }

    const payload = validation.data
    await assertPermission(auth.supabaseAuth, auth.user.id, payload.org_id, "canCreateServices")

    const storage = await insertPilotDataDecayAudit(auth.supabaseAuth, payload)
