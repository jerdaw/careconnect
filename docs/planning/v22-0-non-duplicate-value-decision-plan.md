...
1. External-agent outputs are **hypothesis inputs**, not facts, because those agents did not inspect the CareConnect codebase.
...
1. CareConnect can drift into "directory competition" with 211, where 211 has structural breadth and channel advantages.
2. CareConnect can make claims that are stronger than current evidence (verification freshness, accessibility absolutes, comparative language coverage framing).
3. CareConnect can spend engineering effort on features that improve UI but do not improve real-world service connection outcomes.

Desired strategic position:

1. CareConnect complements 211.
2. CareConnect owns local "last-mile access performance" in Kingston.
3. CareConnect proves value with measurable outcome improvements.
...
2. Durable value if CareConnect becomes frontline workflow infrastructure.
...
2. prevents over-applying CareConnect in scenarios needing immediate human navigation.
...
1. Extend [`types/service.ts`](../../types/service.ts) with operational-status metadata.
...
Add scoped endpoints under [`app/api/v1`](../../app/api/v1):
...
1. [`lib/search/index.ts`](../../lib/search/index.ts)
2. [`lib/search/scoring.ts`](../../lib/search/scoring.ts)
3. [`lib/search/server-scoring.ts`](../../lib/search/server-scoring.ts)
...
1. [`lib/analytics.ts`](../../lib/analytics.ts)
2. [`lib/analytics/search-analytics.ts`](../../lib/analytics/search-analytics.ts)
3. [`app/api/v1/metrics/route.ts`](../../app/api/v1/metrics/route.ts)
...
1. [CareConnect vs 211 Objective Evaluation (2026-02-27)](../evaluation/CareConnect_vs_211_Objective_Evaluation_2026-02-27.md)
2. [CareConnect vs 211 Evidence Matrix (2026-02-27)](../evaluation/CareConnect_vs_211_Evidence_Matrix_2026-02-27.csv)
3. [CareConnect vs 211 Positioning Playbook (2026-02-27)](../evaluation/CareConnect_vs_211_Positioning_Playbook_2026-02-27.md)
