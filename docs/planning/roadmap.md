...
8. Resolve the current dependency-audit findings as bounded maintenance: apply non-breaking fixes where available, split breaking remediation for `next-intl` and `@huggingface/transformers` into a reviewed plan, and do not merge routine Dependabot major-version workflow PRs while required validation is failing.
