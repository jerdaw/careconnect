...
8. **Verify**: `careconnect-offline-v1` database exists and the `services` store contains ~196 items.
...
6. **Verify**: Offline sync runs (watch Console logs and the `careconnect-offline-v1` IndexedDB metadata).
