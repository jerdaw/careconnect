...
- **"Tool Theory":** The Tribunal reinforced the principle that _"If the computer does not think like a man, it is man's fault."_ CareConnect cannot blame the underlying model (Llama-3, etc.). By deploying it, CareConnect adopts its outputs.
- **Vicarious Liability:** The AI is legally analogous to a human volunteer at a help desk. If the AI acts negligently, CareConnect is vicariously liable for that negligence.
- **The "Separate Entity" Fallacy:** CareConnect cannot argue that the AI is a "beta tool" distinct from its core service. CareConnect is the publisher of **every word** the AI generates.

### 1.2 Negligent Misrepresentation Audit (_Queen v. Cognos_)

To establish liability, a plaintiff must satisfy the 5-part test from _Queen v. Cognos Inc._ (1993). CareConnect is at high risk on all counts:

| Test Criteria                | Application to CareConnect Chatbot                                                                                 | Risk Level   |
| :--------------------------- | :----------------------------------------------------------------------------------------------------------------- | :----------- |
| **1. Duty of Care**          | A "special relationship" exists. CareConnect invites reliance by offering a "Help" tool to vulnerable users.       | **HIGH**     |
...
- **Good Samaritan Act Gap:** Ontario's _Good Samaritan Act_ protects individual bystanders. It is legally ambiguous whether it protects an _organization_ systematically deploying a tool. CareConnect is likely viewed as a "service provider," holding it to a higher standard of care.
...
- **The Risk:** The AI creates an "apparent agreement" or acts with "apparent authority." If it says, _"Yes, the shelter has reserved a bed for you,"_ and the user relies on this, CareConnect could be estopped from denying the promise.
- **Impact:** While CareConnect cannot force a shelter to admit the user, it could be liable for damages resulting from the user's reliance (e.g., travel costs, injury from sleeping rough).
...
- **PIPEDA "Commercial Activity":** Even as a non-profit, CareConnect may trigger PIPEDA if it engages in "commercial activities" like selling donor lists or advertising. However, regarding the _chatbot_, since CareConnect does not **collect** the data (it stays on the client), the bulk of PIPEDA does not apply to chat content.
- **Software Distributor Liability:** CareConnect is the _distributor_ of the software. If the client-side code has design vulnerabilities (e.g., Cross-Site Scripting allowing third parties to read the local cache), CareConnect is liable for **negligent design**.
...
- **Health Information Custodian (HIC):** CareConnect is likely _not_ a HIC.
- **Custody:** Under _PHIPA_, liability attaches to "custody or control." Since data never leaves the user's browser, CareConnect never assumes custody.
- **Residual Risk:** If a future update introduces a "server-side logging" feature for debugging (even temporary), CareConnect immediately assumes custody and PHIPA liability. **Strict discipline is required.**

### 2.3 Right to be Forgotten (Erasure)

- **Paradox:** Users may legally demand deletion. CareConnect cannot delete what it doesn't hold.
- **Solution:** The "Nuke History" button. CareConnect must provide a UI control that clears `localStorage` and IndexedDB. This empowers the user to exercise their right to erasure locally.
- **Audit Proof:** CareConnect must maintain a technical "Privacy Whitepapers" documenting the architecture to prove to the IPC that no server-side data exists.
...
- **Action:** CareConnect should explicitly **Map** crisis risks and document **Manage** controls (Safety Preamble). This documentation is evidence of "due diligence" in any liability suit.
...
| **Third-Party Data**             | "CareConnect is not liable for accuracy of third-party listings."                               | Aligns with Competition Act "publisher" defenses.              |
...
>
> 1.  **Identity & Scope:** You are the CareConnect Assistant. You are an automated information retrieval system, **NOT** a social worker, doctor, lawyer, or crisis counselor. You cannot take actions in the real world (e.g., booking beds).
