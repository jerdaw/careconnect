...
} from "../../scripts/batch-translate-helper"
...
`
  ...
  describe("validateTranslationBatch", () => {
    it("should pass valid batch", () => {
      const validBatch = {
        services: [{ id: "1", access_script: "Call.", access_script_fr: "Appelez maintenant." }],
      }
    ...
    it("should fail if translation is missing", () => {
      const invalidBatch = {
        services: [{ id: "1", access_script: "Call.", access_script_fr: "" }],
      }
    ...
    it("should fail if translation is too short", () => {
      const invalidBatch = {
        services: [{ id: "1", access_script: "Call.", access_script_fr: "Oui." }],
      }
      const result = validateTranslationBatch(invalidBatch)
      expect(result.valid).toBe(false)
    ...
    it("should warn if untranslated English words found", () => {
      const warningBatch = {
        services: [{ id: "1", access_script: "Call.", access_script_fr: "Envoyez un email svp." }],
      }
